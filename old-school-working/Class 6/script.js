...
let productPrice = products.map((item) =>
...
let filterCountry = countries.filter((item) =>
  item.toLocaleLowerCase().includes("land")
);
...
let productPriceValuesTypeOf = products.filter(
...
const result =
  countries.reduce(
    (acc, item, i) =>
      acc + (i === countries.length - 1 ? " and " : ", ") + item,
    "Estonia"
...
let everyCountry = countries.every((item) =>
